...
#### Build のスコープを段階化する（推奨）
- **Phase1 (MVP基盤)**: 認証 + M1 メンバー + M2 スキル  
  - ここで「使える」状態を作る（メンバー登録・スキル運用が動く）
- **Phase2**: M3 プロジェクト + M4 勤怠  
  - プロジェクトアサインと日々の勤怠が回り始める
- **Phase3**: M5 請求 + M6 PL/CF  
  - 締め・請求・損益・CFを有効化

※ `docs/requirements/specifications/wireframes.md` の「Phase別MVPワイヤー」を参照し、detail-plan作成時に対象スライスを限定すること。
...
#### デプロイの目安
- Phase1完了: ステージングへデプロイ推奨（メンバー/スキルを実データで入力開始）。本番はまだ不要。
- Phase2完了: 本番初回デプロイ推奨（打刻運用を開始するため）。本番DBに migrate/seed を適用し、Slack設定を本番用に切替。
- Phase3完了: 本番フル運用（請求・PL/CF）。締め前にステージングでリハーサルしてから本番反映。
